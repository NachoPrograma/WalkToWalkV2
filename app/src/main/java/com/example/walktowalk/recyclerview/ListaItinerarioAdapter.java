...
public class ListaItinerarioAdapter extends RecyclerView.Adapter<ItinerarioViewHolder>{
    ...
    public void onBindViewHolder(@NonNull ItinerarioViewHolder holder, int position) {
        if(listaItinerario!=null) {
            ...
            holder.img_itinerario.setImageResource(itinerario_actual.getImagenResId());
            if(this.listaFotosItinerario != null)
            {
                for(Fotos fc: this.listaFotosItinerario)
                {
                    if(fc.getIdfoto().equals(itinerario_actual.getIdciudad()))
                    {
            ...
            InputStream inputStream = p.getResources().openRawResource(itinerario_actual.getImagenResId());
            Bitmap bitmap = BitmapFactory.decodeStream(inputStream);
            holder.img_itinerario.setImageBitmap(bitmap);
        }
    }
    @Override
