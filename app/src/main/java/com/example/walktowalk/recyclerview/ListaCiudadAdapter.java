...
public class ListaCiudadAdapter extends RecyclerView.Adapter<CiudadViewHolder>{
    ...
    public void onBindViewHolder(@NonNull CiudadViewHolder holder, int position) {
        if(listaCiudad!=null) {
            ...
            holder.img_ciudad.setImageResource(ciudad_actual.getImagenResId());
            if(this.listaFotosCiudades != null)
            {
                for(Fotos fc: this.listaFotosCiudades)
                {
                    if(fc.getIdciudad().equals(ciudad_actual.getId()))
                    {
            ...
            InputStream inputStream = r.getResources().openRawResource(ciudad_actual.getImagenResId());
            Bitmap bitmap = BitmapFactory.decodeStream(inputStream);
            holder.img_ciudad.setImageBitmap(bitmap);
        }
    }
    @Override
